...
public class Verificador implements Serializable {
     ...
     *
     */
    private static final long serialVersionUID = -1715556843790152132L;
    ...
    public void verificaLoginExistente(String login) throws MeLevaException {
        Iterator<Usuario> it = ControleDeUsuarios.getInstance().getUsuariosCadastradosPorLogin().iterator();
        while (it.hasNext()) {
    ...
    public void verificaEmailExistente(String email) throws MeLevaException {
        Iterator<Usuario> iterador = ControleDeUsuarios.getInstance().getUsuariosCadastradosPorLogin().iterator();
     ...
     *
     * @param sessoesAbertas - Sessões abertas no sistema.
     * @param idSessao - Identificador de sessão.
     * @throws MeLevaException - - Exceções para o caso dos dados inseridos
     * sejam inválidos.
     */
    ...
    public void verificaData(String data, String hora) throws MeLevaException {
        ...
        if (hora != null && !hora.equals("") && hora.matches("[0-9]{1,2}:[0-9]{2}")) {
     ...
     *
     * @param pontos - ponto de encontro.
     * @throws MeLevaException - Exceções para o caso dos dados inseridos sejam
     * inválidos.
     */
     ...
     * @param sugestoesPontoDeEncontro - Sugestoes de ponto de encontro.
     * @throws MeLevaException - Exceções para o caso dos dados inseridos sejam
     * inválidos.
     */
    public void verificaContemNegociacao(AbstractCarona carona,
            ...
            throws MeLevaException {
        Iterator<NegociacaoDePonto> sugestaoIt = sugestoesPontoDeEncontro.values().iterator();

        while (sugestaoIt.hasNext()) {
            NegociacaoDePonto nextSugestao = sugestaoIt.next();
    ...
    public void verificaCaronaOferecida(Usuario userReview, String idCarona)
            throws MeLevaException {
        Iterator<AbstractCarona> caronaOfertada = userReview.getCaronasOfertadas().iterator();
    ...
    public void verificaCaronaPedida(Usuario user, String iDCarona)
            throws MeLevaException {
        Iterator<AbstractCarona> caronaPedida = user.getCaronasPedidas().iterator();
        Carona caronaOK = null;
     ...
     *
     * @param horaInicio - Hora inicial.
     * @param horaCarona - Hora da carona oferecida.
     * @param horaFim - Hora do final da espera.
    ...
    public void verificaDestinatario(String destinatario) throws MeLevaException {
        verificaEmail(destinatario);
        List<Usuario> usuarios = ControleDeUsuarios.getInstance().getUsuariosCadastradosPorLogin();
        Iterator<Usuario> it = usuarios.iterator();
        String email = null;
        while (it.hasNext()) {
            Usuario user = it.next();
            if (user.getEmail().trim().equals(destinatario)) {
                email = destinatario;
                break;
            }
    ...
    public void verificaEmailNome(String destinatario, String nomeCompleto) {
        verificaEmail(destinatario);
        List<Usuario> usuarios = ControleDeUsuarios.getInstance().getUsuariosCadastradosPorLogin();
        Iterator<Usuario> it = usuarios.iterator();
        String email = null;
        while (it.hasNext()) {
            Usuario user = it.next();
            if (user.getEmail().trim().equals(destinatario)) {
                if (user.getNome().equals(nomeCompleto)) {
                    email = destinatario;
                } else {
                    throw new MeLevaException(MSGDeExcecao.MSG_ERRO_NOME_INVALIDO);
                }
